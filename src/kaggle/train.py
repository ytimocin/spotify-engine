...
def split_playlist_tracks(
    graph, holdout_size: int = 5, val_ratio: float = 0.2, seed: int = 42, max_playlists: int = None
    ...
    Args:
        ...
        seed: Random seed
        max_playlists: Maximum number of playlists to use (for faster training)
    ...
    if max_playlists is not None:
        playlist_indices = list(playlist_to_tracks.keys())
        random.shuffle(playlist_indices)
        playlist_indices = playlist_indices[:max_playlists]
        playlist_to_tracks = {idx: playlist_to_tracks[idx] for idx in playlist_indices}
        logger.info(f"Limited to {len(playlist_to_tracks)} playlists for training")
...
def create_training_graph(graph, train_tracks: Dict[int, List[int]]):
    ...
    train_graph[("playlist", "contains", "track")].edge_index = train_edge_index
    # Create reverse edges by flipping the indices
    train_graph[("track", "in_playlist", "playlist")].edge_index = torch.stack(
        [train_edge_index[1], train_edge_index[0]]  # track indices  # playlist indices
    )
    ...
    for epoch in range(num_epochs):
        ...
        for i in tqdm(range(0, len(train_playlists), batch_size), desc=f"Epoch {epoch + 1}"):
            ...
            for playlist_idx in batch_playlists:
                ...
                pos_tracks = random.sample(
                    list(playlist_tracks_set), min(5, len(playlist_tracks_set))
                )
...
def main():
    ...
    parser.add_argument(
        "--holdout-size", type=int, default=5, help="Tracks to hold out per playlist"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Max number of playlists to use (for faster training)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
