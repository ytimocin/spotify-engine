...
def load_kaggle_data(kaggle_dir: Path):
    ...
    playlists_df = pd.read_csv(playlist_path, 
...
def create_playlist_entities(playlists_df: pd.DataFrame):
    ...
    playlists_df['playlist_id'] = (
        playlists_df['user_id'].astype(str) + '_' + 
        playlists_df['playlistname'].astype(str)
    )
    
    # Get unique playlists
    playlists = playlists_df.groupby('playlist_id').agg({
        'user_id': 'first',
        'playlistname': 'first',
        'trackname': 'count'
    }).reset_index()
...
def match_tracks(playlists_df: pd.DataFrame, tracks_df: pd.DataFrame):
    ...
    playlists_df['track_key'] = (
        playlists_df['trackname'].str.lower().str.strip() + '_' +
        playlists_df['artistname'].str.lower().str.strip()
    )
    
    tracks_df['track_key'] = (
        tracks_df['name'].str.lower().str.strip() + '_' +
        tracks_df['artists_parsed'].str.lower().str.strip()
    )
    
    # Match tracks
    # Select available columns from tracks_df
    feature_cols = ['id', 'track_key', 'name', 'artists', 'artists_parsed',
    ...
    matched = playlists_df.merge(
        tracks_df[feature_cols],
        on='track_key',
        how='inner'
    )
...
def infer_genres(tracks_df: pd.DataFrame):
    ...
    genres = pd.DataFrame({
        'genre_name': tracks_df['genre'].unique()
    })
...
def save_processed_data(output_dir: Path, playlists, tracks, artists, genres, 
                       playlist_tracks, track_genres, artist_genres):
...
def main():
    ...
    parser.add_argument(
        "--sample",
        type=int,
        help="Sample N playlists for testing"
    )
    ...
    if args.sample:
        ...
        sampled = np.random.choice(unique_playlists, size=min(args.sample, len(unique_playlists)), replace=False)
    ...
    track_cols = ['id', 'name', 'artist_id', 'duration_ms',
    ...
    new_col_names.extend(['danceability', 'energy', 'acousticness',
    ...
    save_processed_data(
        ...
        playlists, tracks, artists, genres,
        playlist_tracks, track_genres, artist_genres
    )
