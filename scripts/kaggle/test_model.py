...
def display_playlist_info(graph, playlist_idx: int):
    ...
    print(f"Features: track_count={playlist_features[0]:.2f}, "
    ...
    if len(track_indices) > 0:
        ...
        for i, track_idx in enumerate(track_indices[:5]):
            track_features = graph["track"].x[track_idx]
            print(f"  Track {track_idx}: energy={track_features[1]:.2f}, "
...
def test_recommendations(model: PlaylistGAT, graph, playlist_idx: int, k: int = 10):
    ...
    for i, (track_idx, score) in enumerate(zip(rec_tracks, rec_scores)):
        ...
        print(f"   Features: energy={track_features[1]:.2f}, "
              f"valence={track_features[5]:.2f}, "
...
def test_explanations(model: PlaylistGAT, graph, playlist_idx: int, track_idx: int):
    ...
    if explanation.get("similar_tracks_in_playlist"):
        ...
        for track_info in explanation["similar_tracks_in_playlist"]:
            print(f"  - Track {track_info['track_idx']}: "
...
def main():
    ...
    parser.add_argument(
        "--playlist", type=int, default=0, help="Playlist ID to test"
    )
    parser.add_argument(
        "--top-k", type=int, default=10, help="Number of recommendations"
    )
    parser.add_argument(
        "--explain", action="store_true", help="Show recommendation explanations"
    )
    ...
    existing_tracks = display_playlist_info(graph, args.playlist)
